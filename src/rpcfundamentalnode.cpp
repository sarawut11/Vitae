...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "stop")
    {
        ...
        if(activeMasternode.status == ACTIVE_MASTERNODE_STOPPED) return "successfully stopped masternode";
        if(activeMasternode.status == ACTIVE_MASTERNODE_NOT_CAPABLE) return "not capable masternode";
    ...
    /*if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            CKey keyMasternode;
                        ...
                        CMasternode* pmn = m_nodeman.Find(pubKeyMasternode);
                        if(pmn == NULL)
                        {
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() == 1){
        strAddress = params[0].get_str().c_str();
    } else {
        throw runtime_error(
...
UniValue masternodedebug (const UniValue& params, bool fHelp)
{
    ...
    if (activeMasternode.status != ACTIVE_MASTERNODE_INITIAL)
        return activeFundamentalnode.GetStatus();
